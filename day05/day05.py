...
def get_parameters(param: int) -> Tuple[int, List[int]]:
    ...
    first_param = param // 100 % 10
    second_param = param // 1000 % 10
    third_param = param // 10000 % 10

    return opcode, [first_param, second_param, third_param]

def assign_values(prog: List[int], params: List[int], idx: int) -> Tuple[int, int]:
    if params[0] == 0:
        v1 = prog[prog[idx + 1]]
    else:
        v1 = prog[idx + 1]
...
def test_diagnostic(prog: List[int], input: List[int]=[1]) -> List[int]:
    idx = 0
    output = []

    while prog[idx] != 99:
        code, params = get_parameters(prog[idx])
        if code == 1:
            v1, v2 = assign_values(prog, params, idx)
            prog[prog[idx + 3]] = v1 + v2
            idx += 4
            ...
            if params[0] == 0:
                output.append(prog[prog[idx + 1]])
            else:
                output.append(prog[idx + 1])
            idx += 2
